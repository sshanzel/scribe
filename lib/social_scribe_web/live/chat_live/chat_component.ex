defmodule SocialScribeWeb.ChatLive.ChatComponent do
  @moduledoc """
  LiveComponent for the floating chat interface.
  Includes the chat bubble, panel with thread list, and message display.
  """
  ...
  def update(assigns, socket) do
    ...
    socket =
      if connected?(socket) && socket.assigns.threads == [] do
        assign(socket, :threads, Chat.list_threads(assigns.current_user))
      else
        socket
      end
  ...
  def handle_event("send_message", %{"message" => content}, socket) do
    ...
    if content == "" || socket.assigns.selected_contact == nil do
      ...
      # Generate response synchronously for now (can be made async later)
      socket =
        socket
        |> assign(:loading, true)
        |> assign(:message_input, "")
